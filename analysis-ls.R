...
ext_lin.fit <- ds.lmerSLMA(
  ...
  (1|child_id_int)",
  datasources = conns[ext_pc_coh_lin])
...
int_lin.fit <- ds.lmerSLMA(
  ...
  (1|child_id_int)",
  datasources = conns[int_pc_coh_lin])
...
ext_lin_pred_coh_se <- ext_lin_pred_coh %>%
  pmap(function(study_ref, age, ...){
    ...
    vcov <- ext_pc_lin.fit$output.summary[[study_ref]]$vcov
    C <- c(0, 1, 0, age)
...
int_lin_pred_coh_se <- int_lin_pred_coh %>%
  pmap(function(study_ref, age, ...){
    ...
    vcov <- int_pc_lin.fit$output.summary[[study_ref]]$vcov
    C <- c(0, 1, 0, age)
    std.er <- sqrt(t(C) %*% vcov %*% C)
    out <- std.er@x
    return(out)}) %>%
