...
## ---- List of cohorts --------------------------------------------------------
inc_cohorts <- unique(c(int_coh, ext_coh, adhd_coh, asd_coh, lan_coh, nvi_coh))
sort(inc_cohorts)
## ---- Number of cohorts ------------------------------------------------------
n_cohorts <- length(inc_cohorts)
...
## ---- Baseline sample --------------------------------------------------------
baseline_n <- ds.dim("mh_df", type = "c")[[1]][[1]]
...
################################################################################
# Table S2: Instruments used
################################################################################
instruments <- dh.getStats(
  df = "analysis_df", 
  vars = inst.vars)

instruments.tab <- instruments$categorical %>%
  dplyr::filter(cohort != "combined" & value != 0) %>%
  ...
  map(function(x){
    x %>%
      mutate(
        instr_col = pull(., instrument) %>% paste(., collapse = ", ")) %>%
      slice(1)
  }) %>%
  bind_rows() %>%
  dplyr::select(cohort, instr_col, variable) %>%
  pivot_wider(
    names_from = "variable", 
    values_from = "instr_col") %>%
  dplyr::select(cohort, ext_instr_, int_instr_, adhd_instr_, asd_instr_, 
                lan_instr_, nvi_instr_) %>%
  mutate(across(everything(), ~replace_na(.x, ""))) %>%
  arrange(cohort)

write_csv(instruments.tab, file = here("tables", "instruments.csv"))
